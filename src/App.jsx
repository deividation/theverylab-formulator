...
{showSample&&<div style={{marginTop:"2rem",background:"#fff",border:"1.5px solid rgba(0,0,0,0.08)"}}>
  <div style={{background:"#9F132D",color:"#fff",padding:"1.5rem 2rem"}}><div style={{fontSize:"1.4rem",fontWeight:700}}>🧪 {t.sampleTitle}</div></div>
  <div style={{padding:"1.8rem"}}>
    <p style={{fontSize:"0.88rem",marginBottom:"0.8rem",opacity:0.75}}>{t.sampleDesc}</p>
    <div style={{background:"#F6D4D8",borderLeft:"3px solid #9F132D",padding:"0.7rem 1rem",marginBottom:"1.5rem",fontSize:"0.82rem",fontWeight:600}}>{t.samplePrice}</div>
    <div style={{display:"flex",flexDirection:"column",gap:"0.8rem",marginBottom:"1.5rem"}}>
      {[{l:t.sampleNameLabel,k:"sName"},{l:t.sampleEmailLabel,k:"sEmail"},{l:t.sampleCompanyLabel,k:"sCompany"}].map(({l,k})=>(
        <div key={k}><div style={{fontSize:"0.72rem",opacity:0.5,marginBottom:"0.3rem",textTransform:"uppercase",letterSpacing:"0.06em"}}>{l}</div>
        <input type="text" style={{width:"100%",border:"1.5px solid rgba(0,0,0,0.12)",padding:"0.7rem 1rem",fontFamily:"'Afacad',sans-serif",fontSize:"0.9rem",outline:"none"}} /></div>
      ))}
    </div>
    <div style={{display:"flex",gap:"1rem"}}>
      <button className="btn ghost" onClick={()=>setShowSample(false)}>{t.back}</button>
      <button className="btn">{t.sampleSubmit}</button>
    </div>
  </div>
</div>}
{showWaitlist&&<div style={{marginTop:"2rem",background:"#fff",border:"1.5px solid rgba(0,0,0,0.08)"}}>
  <div style={{background:"#000",color:"#fff",padding:"1.5rem 2rem"}}><div style={{fontSize:"1.4rem",fontWeight:700}}>⏳ {t.waitlistTitle}</div></div>
  <div style={{padding:"1.8rem"}}>
    <p style={{fontSize:"0.88rem",marginBottom:"1.5rem",opacity:0.75}}>{t.waitlistDesc}</p>
    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"0.8rem",marginBottom:"1.5rem"}}>
      <div style={{border:"1.5px solid rgba(0,0,0,0.08)",padding:"1rem",textAlign:"center"}}>
        ...
        <div style={{fontSize:"2rem",fontWeight:700,color:"#9F132D",lineHeight:1}}>#7</div>
      </div>
      <div style={{border:"1.5px solid rgba(0,0,0,0.08)",padding:"1rem",textAlign:"center"}}>
        ...
        <div style={{fontSize:"2rem",fontWeight:700,lineHeight:1}}>4 <span style={{fontSize:"1rem",fontWeight:400,opacity:0.5}}>{t.waitlistWeeks}</span></div>
      </div>
    </div>
    <div style={{display:"flex",gap:"0.8rem",alignItems:"flex-end",marginBottom:"1rem"}}>
      <div style={{flex:1}}><div style={{fontSize:"0.72rem",opacity:0.5,marginBottom:"0.3rem",textTransform:"uppercase",letterSpacing:"0.06em"}}>{t.waitlistEmailLabel}</div>
      <input type="email" placeholder="hello@yourbrand.com" style={{width:"100%",border:"1.5px solid rgba(0,0,0,0.12)",padding:"0.7rem 1rem",fontFamily:"'Afacad',sans-serif",fontSize:"0.9rem",outline:"none"}} /></div>
      <button className="btn">{t.waitlistSubmit}</button>
    </div>
    <button className="btn ghost" onClick={()=>setShowWaitlist(false)} style={{width:"100%"}}>{t.back}</button>
  </div>
</div>}
</div>
    </div>
