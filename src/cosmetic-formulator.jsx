...
export default function App() {
  ...
  const submit = async () => {
    ...
    const catMap = { women:"moteru", men:"vyru", pets:"gyvunu", fragrance:"kvepalai", home:"namu kvapai" };
    const skinCtx = ctx();
    const skinInfo = form.skin.length > 0 ? skinCtx.skinLabel + ": " + form.skin.join(", ") : "";
    const prompt = "Tu esi ekspertas kosmetikos formuluotojas private label laboratorijai the very lab.\n\n"
      + "Kategorija: " + (catMap[form.cat] || form.cat) + "\n"
      + "Produkto tipas: " + form.type + "\n"
      + (skinInfo ? skinInfo + "\n" : "")
      + "Norimi efektai: " + form.effects.join(", ") + "\n"
      + "Vengtini ingredientai: " + (form.avoid.length > 0 ? form.avoid.join(", ") : "nera") + "\n"
      + "Kiekis: " + pricing.qty + " vnt\n"
      + (packaging.length > 0 ? "Pasirinktos pakuotes: " + packaging.map(id => { const p = PACKAGING_CATALOG.find(x=>x.id===id); const sz = selectedSizes[id]||(p&&p.sizes[0])||""; return p ? p.name+" "+sz : ""; }).filter(Boolean).join(", ") + "\n" : "")
      + "Papildoma info: " + (form.brief || "nera") + "\n\n"
      + "Pateik formulavimo krypti lietuviu kalba."
      + (form.cat === "pets" ? " Ingredientai turi buti saugus gyvunams." : "")
      + (isFH ? " Sutelk demesi i kvapines natas (virsutines/sirdingas/bazines)." : "")
      + " Naudok sekcijas:\n"
      + (isFH
        ? "FORMULAVIMO KRYPTIS\nKVAPINES NATOS\nREKOMENDUOJAMOS ZALIAVOS\nKONCENTRACIJA\nREGULIACINES PASTABOS\nISKIRTINUMAS"
        : "RECEPTUROS KRYPTIS\nAKTYVIEJI INGREDIENTAI (4-6 su INCI)\nBAZINIAI INGREDIENTAI\nKONCENTRACIJA\nREGULIACINES PASTABOS\nISKIRTINUMAS");
    try {
      ...
      setStep(8);
    } catch(e) { setError("Klaida. Bandykite dar karta."); }
      ...
      {step === 0 && (
        <div style={{ background:C.red, padding:"4rem 2rem 3.5rem", textAlign:"center" }}>
          <div style={{ maxWidth:"600px", margin:"0 auto" }}>
            <p style={{ fontSize:"0.72rem", letterSpacing:"0.2em", textTransform:"uppercase", color:"rgba(255,255,255,0.6)", marginBottom:"1rem" }}>Private Label Formulavimas</p>
            <h1 style={{ fontFamily:"'Afacad',sans-serif", fontSize:"clamp(2.2rem,6vw,3.5rem)", fontWeight:700, color:C.white, lineHeight:1.1, marginBottom:"1.2rem", letterSpacing:"-0.01em" }}>
              Sukurkite savo<br />receptūros kryptį
            </h1>
            <p style={{ fontSize:"1rem", color:"rgba(255,255,255,0.7)", lineHeight:1.6, maxWidth:"420px", margin:"0 auto" }}>
              Atsakykite į kelis klausimus ir gaukite AI parengtą formulavimo kryptį bei indikatyvią kainą.
            </p>
          </div>
        </div>
      )}
